...
class Service {
  ...
  handleRequest(req, data, res) {
    ...
    if (req.url === '/bots') {
      ...
      if (reqPath.length > 3 && reqPath[1] === 'bots'){
        if (reqPath[3] === 'messages') {
          this.handleMessages(req, data, res, reqPath[2]);
        }
        else if (reqPath[3] === 'gitlab') {
          // get git request
          console.log("Get git. Ready to hook!");
          // send response first so we don't keep gitlab waiting
          Service.sendResponse(res, 200);

          var botId = reqPath[2];
          if (!{}.hasOwnProperty.call(this.bots, botId)) {
            this.loadState(botId).then((result) => {
                if (!result) {
                  console.log(`Can't initialize bot with ID ${botId}`)
                } 
                else {
                  if (typeof this.cb === 'function') {
                    this.cb(this.bots[botId].botInstance);
                  }
                  gitlab.handleMessage(req, data, this.bots[botId].botInstance);
                }
            });
          }
          else {
